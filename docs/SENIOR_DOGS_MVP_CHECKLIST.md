...
38. **Sibling routes checked, none fixed — flagged for a separate discussion, not touched.** Same Sep 2 investigation checked every other route the `9cd0be7` bulk-import commit introduced for the same danger shape (dead in frontend + live-schema query + no dedicated auth). Found real siblings, but with meaningfully different risk profiles, not silently bundled into item 37's fix: **`GET /api/governance/stats`** — the closest true sibling (dead in the frontend too, live-schema, same incidental-only protection), but returns aggregate counts only (no per-record data), and its frontend page (`governance.html`) is now just a 12-line redirect stub to `privacy.html` — reads as an intentionally-public feature whose UI was later replaced, not a forgotten debugging leftover. **`POST /api/test-email`** and **`POST /api/test-churn-detection`** — unauthenticated, but a different risk *type* (an arbitrary-recipient real-email-send capability, and an unauthenticated trigger that processes every real dog but returns only counts and always emails the internal `SENDGRID_FROM_EMAIL`, never a real owner) — both are actively-referenced, intentionally-kept internal QA tools elsewhere in this project's own Build Log history, not accidental orphans. **`POST /api/age-bella-data`** / **`POST /api/clear-bella-alert`** — unauthenticated, but hardcoded to one fixed, almost-certainly-nonexistent test-dog UUID, so blast radius is structurally bounded to that one ID, unlike `get-all-dogs`'s unbounded "every row" shape. Already-known dead-legacy-schema routes (`/api/admin/users`, `/api/user/:user_id`, `/api/pet/:pet_id/progress`, `/api/signups`) confirmed still non-functional (query the fully-dropped `users`/`pets`/`survey_*` schema), carrying none of this real exposure regardless of auth — not re-investigated further. Full detail, including the exact reasoning for each, in the Build Log's Sep 2 (continued) entry.
