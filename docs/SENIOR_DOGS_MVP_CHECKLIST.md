...
**As of September 3, 2026:** Pre-beta, `SITE_PASSWORD`-locked, zero real users — still true today, and still the single biggest fact about where this project stands (see item 47). Since this section's own historical narrative stopped being kept current day-by-day (past Aug 23, per the note below), the project has shipped: the STEP P11 breed guide expansion (30→75 breeds); a full RLS remediation after every table was found publicly writable; the entire brand identity rollout (favicon, header monogram, hero graphic); category-level medication/supplement tracking, fully verified across every entry path (baseline signup, direct add, weekly update, stop); a real email-unsubscribe mechanism and a churn-email cadence fix (was re-firing up to ~7×/missed week, now capped at 1); a US-residents-only signup gate (ZIP validation, `libphonenumber-js` US/Canada-aware phone validation, Twilio account-level geo-restriction); a real Twilio credential leak (Sep 1, rotated same session) that produced this project's standing security-sweep cadence (documented in `CompanionCommons_Project_Overview_1.md`); a full security audit that found and removed 4 live, unauthenticated debug/admin routes including a full-database dump and an arbitrary-recipient email-send capability; a complete link-revocation and access-token system making every dog/owner link genuinely revocable for the first time, live-verified in production; and a full Privacy Policy/Terms of Service rewrite, published live, with a matching FAQ alignment pass. **Items 40–47 below cover everything from this session specifically** (link revocation, legal pages, FAQ pass, and the real still-open items — the qs/body-parser vulnerability, the deprioritized `authorizeOwnerScope` timing comparison, two untracked/undecided doc loose ends, and beta recruiting itself). `CompanionCommons_Build_Log.md` has the full day-by-day detail; this checklist's NEXT STEP list below is the current, accurate punch list.
...
44. **`authorizeOwnerScope`'s `access_token` comparison uses `===`, not `crypto.timingSafeEqual` — deliberately deprioritized, not forgotten.** Same *class* of non-constant-time string comparison as the session-cookie signature bug fixed in item 40, found while implementing that exact fix and consciously left alone in the same pass. Reasoning: `access_token` is a 36-character random UUID this app already transmits in plaintext by design, in every mailed/texted link — its threat model is "hard to guess," not "resistant to being observed," unlike the session signature (a value that should never leave the server at all, and where the cookie's own plaintext `ownerId` gave a real, non-random anchor to time against). A timing side-channel does essentially nothing to make guessing a 36-char random value more tractable. Revisit if this app's threat model ever changes to include an attacker able to make a very large volume of precisely-timed requests against a single guessed-`ownerId`+partial-token combination cheaply — worth checking against the current per-route rate-limiting posture first (see item 22, `/checkins/:owner_id` has none). Not urgent. Full detail in `Link_Revocation_Build.md`'s Backlog section.

45. **`part3_medication_sheets_tabs.diff` — a real, working patch sitting untracked and unapplied, pending John's decision.** Built and verified Sep 1 (see item 35): new Medications/MedicationUpdates Google Sheets tabs, wired into all 4 real medication call sites, with the standing free-text-export rule already correctly enforced (the `note`/`condition_treated_other_detail` fields never even reach the append function's parameters). Deliberately held back before commit because John wants to decide separately whether medication data belongs in Google Sheets at all, not because anything about the patch itself is unfinished. Confirmed still sitting untracked in the repo root as of Sep 3, still `git apply --check`-clean per the Sep 2 audit. Real open item: either apply it (a decision to ship), or delete it (a decision not to) — it shouldn't sit untracked indefinitely as a silent loose end.

46. **`docs/LEGAL-DRAFTS/Privacy_Terms_Rewrite_Outline (1).md` — untracked, now superseded, no decision made on its disposition.** The working outline behind the Sep 3 Privacy Policy/Terms of Service rewrite (item 41) — its own "Summary of Real Open Items" list is now stale in at least one place (item 1, the consent-record claim, was corrected during this session's investigation; see item 41 and the Build Log). The rewrite it was written to drive has shipped and is live. Real open question, not yet decided: commit it to the repo for historical record (matching how `docs/LEGAL-DRAFTS/Privacy_Policy_Draft.md`/`Terms_of_Service_Draft.md` are also still untracked, same open question), or delete it now that its job is done. Low priority, purely a repo-hygiene loose end.

47. **Begin recruiting for the beta — still not started as of Sep 3, 2026.** Restated from item 6, since it's the single largest genuinely-open item on this whole list and easy to lose among the security/legal work above: 8 real strangers matching the actual target persona, plus 2–3 close friends, per the locked Aug 20 beta-structure decision. The site remains `SITE_PASSWORD`-locked (item 2) and has zero real users. Nothing about the security or legal-pages work done this session changes this — if anything, both were prerequisites worth having in place *before* real strangers start signing up, not blockers to recruiting itself. No technical blocker remains that this checklist is aware of; this is a real decision and a real outreach effort, not a build task.
