...
## STEP P11: Breed Guide Expansion — Progressive Unlock + Breed-Matching Fix ⏳ PLANNING COMPLETE (Aug 24)
...
**Status:** planning-only pass complete (this session, Aug 24) — decisions locked, 5-stage plan proposed (schema check, breed-matching fix, unlock-structure rebuild, personalization content, verification pass), no code written yet. Build starts in a future session.
...
14. **Breed-matching coverage gap** — found during the weight-chart build, originally flagged as a near-term discussion. **Now being addressed as part of STEP P11 (Aug 24)** — see `Breed_Guide_Expansion_Build.md` for the locked plan (layered exact/substring/alias/fuzzy matching plus fall-through logging). Planning complete, not yet built. Original note, kept for context: breed is a free-text field at signup (no dropdown, no autocomplete), and `getBreedGuide()` matches only via exact string after trim/lowercase against a fixed list of ~30 curated breeds. Anything that doesn't exact-match (a typo, "Lab" instead of "Labrador Retriever", "Golden retriever mix", or a real breed genuinely outside the curated list) silently falls through to the generic fallback, which has no numeric weight range at all. This affects TWO existing features silently, not something new: `isSeniorForBreed` (senior-by-breed flag) has had this same limitation the whole time, and `isOverweightForBreed` inherits it. No error surfaces anywhere when this happens — a real, hard-to-estimate share of beta dogs could get zero benefit from either flag with nothing telling anyone it happened.
15. **STEP P11 (Breed Guide Expansion)** — planning complete (Aug 24), see `Breed_Guide_Expansion_Build.md` for the full locked plan: progressive unlock at weeks 2/4/8/12 (replacing the current single week-2 unlock), the breed-matching fix from item 14 above, and weeks 8/12 personalized content built from existing helpers (`calculateCurrentStreak`, an extracted-to-shared `describeJourneyTrend`) rather than new trend logic. No code written yet, no new database columns or migrations anywhere in this project. Not started.
