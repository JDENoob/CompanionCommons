...
**As of September 4, 2026:** Pre-beta, `SITE_PASSWORD`-locked, zero real users — still true today, and still the single biggest fact about where this project stands (see item 47). Since this section's own historical narrative stopped being kept current day-by-day (past Aug 23, per the note below), the project has shipped: the STEP P11 breed guide expansion (30→75 breeds); a full RLS remediation after every table was found publicly writable; the entire brand identity rollout (favicon, header monogram, hero graphic); category-level medication/supplement tracking, fully verified across every entry path (baseline signup, direct add, weekly update, stop); a real email-unsubscribe mechanism and a churn-email cadence fix (was re-firing up to ~7×/missed week, now capped at 1); a US-residents-only signup gate (ZIP validation, `libphonenumber-js` US/Canada-aware phone validation, Twilio account-level geo-restriction); a real Twilio credential leak (Sep 1, rotated same session) that produced this project's standing security-sweep cadence (documented in `CompanionCommons_Project_Overview_1.md`); a full security audit that found and removed 4 live, unauthenticated debug/admin routes including a full-database dump and an arbitrary-recipient email-send capability; a complete link-revocation and access-token system making every dog/owner link genuinely revocable for the first time, live-verified in production; a full Privacy Policy/Terms of Service rewrite, published live, with a matching FAQ alignment pass; and, since Sep 3, both real backlog items from that link-revocation sweep closed (the `authorizeOwnerScope` timing side-channel matched to the earlier session-cookie fix, and the `qs`/`body-parser` moderate vulnerability resolved via a targeted `overrides` pin — no Express 5 upgrade needed), plus a real signup-form gap closed on both forms (genuine support for dogs under 1 year old, an in-context notes disclosure, and the treatment-checkbox/medication-entry merge — see item 48). **Items 40–48 below cover everything since link revocation went live** (link revocation, legal pages, FAQ pass, the two now-closed security backlog items, the signup-form fixes, and the real still-open items — the bit-rotted medication-Sheets diff, the still-untracked Privacy Policy/Terms of Service draft files, and beta recruiting itself). `CompanionCommons_Build_Log.md` has the full day-by-day detail; this checklist's NEXT STEP list below is the current, accurate punch list.
...
**Last Updated (this narrative section):** August 23, 2026 (later same day) — **STEP P10 built and shipped, all 6 stages**, following up on the planning pass captured earlier this same day (see entry below). Full detail in `Health_Instrument_Redesign_Build.md`. Schema migration + shared helpers, both signup forms, both check-in surfaces, the sign-flip work across the dashboard/insight/alert logic, Google Sheets headers, and a real verification pass — signup, single- and multi-week check-in, a real multi-dog-owner scenario (second dog via Add Another Dog, combined check-ins page, dashboard switcher confirmed both with and without a session), resend-dashboard-link, and a genuine second cadence cycle exercising real checkin-to-checkin trend text and alert logic. Both migrations run against real Supabase. Two real bugs found and fixed along the way, neither part of the original plan: a pre-existing legacy DB constraint (`mobility_check`) still enforcing the old 1-8 range on one column, found via the first real signup after the main migration ran; and a falsy-zero bug in the dashboard's peer-comparison card, found during self-review. One manual follow-up remains for John: add "(0-10)" to the Google Sheet's live score-column headers.

**Real gap, found and flagged during the Sep 2 doc-sync audit, not fixed here:** this day-by-day narrative (the "Completed [date] session" paragraphs above) was never kept current past Aug 23 — roughly 10 more days and dozens of real Build Log entries (STEP P11's breed-guide expansion, the RLS remediation, brand identity rollout, medication tracking, email unsubscribe, the US-residents gate, and the medication-event milestone feature among them) exist with no corresponding paragraph here. Rewriting this section to match would be a large, mechanical transcription of the Build Log rather than new information — the **NEXT STEP list immediately below and `CompanionCommons_Build_Log.md` are the current, accurate source of truth**; this narrative section stops being reliable past Aug 23 and should be read as historical scene-setting only, not a live status.
...
2. **Decide when to remove `SITE_PASSWORD`** — still genuinely just a decision, now explicitly gated on its own full security-sweep moment (credential inventory, RLS check, dependency audit, endpoint auth review — the same full-sweep template already run before the Sep 3 link-revocation deploy) rather than a bare go/no-go, per the standing security cadence documented in `CompanionCommons_Project_Overview_1.md`
...
17. ~~Medication/supplement change tracking (category-level, structured)~~ — **BUILT (Sep 1), migration-gated verification still pending.** Full category-level system shipped: new `medications` (repeating, category/condition/source/dates, zero owner-identifying columns by construction) and `medication_weekly_updates` tables, progressive-disclosure weekly check-in UX (0 active → question hidden; 1 active → single yes/no, auto-attributed; 2+ active → yes/no then "which one?"), repeating add-medication UI on both baseline signup surfaces, and a direct `POST /api/medications` + `POST /api/medications/:id/stop` pair for adding/stopping outside the weekly flow. Dose remains permanently rejected (not deferred — clinically meaningless self-reported data, pure liability); drug name remains correctly deferred pending real autocomplete + lawyer review (item 18, unchanged), but the schema is now "separation-compatible" with the future identifiable/de-identifiable architecture split by construction, so adding a name column later is additive, not a redesign. `migration_add_medications.sql` written, not yet run (per standing rule). Validation logic and every new/modified endpoint's reachability were verified live against the real server; the full 0→1→2-active→stopped live sequence (task's own item 8) is explicitly NOT yet verified, since it needs the migration actually running against real data — flagged as item 33 below, same two-session pattern already used for every other migration-gated feature this project has shipped. Full design discussion and verification detail in the Build Log's Sep 1 "Medication/Supplement Category-Level Tracking, Built" entry.
...
20. **Adverse events and lifestyle as tracked data categories (near-term discussion, not scoped).** Surfaced while reviewing the business-architecture reference doc against the current schema: both are listed as expected inputs to the longitudinal dataset, but neither is currently a tracked field anywhere in the signup or check-in flow. "Adverse events" needs a real definition (a supplement/medication reaction? an injury? tied to the medication-change tracking work in items 17/18, or a separate category?) before it can be scoped. "Lifestyle" is even vaguer as stated — diet_type already exists at signup, but broader lifestyle factors (exercise environment, other pets in household, etc.) aren't defined anywhere yet. Not scoped, not estimated, needs real definition work before it can even be sized.
21. ~~Fabricated comparison data found live on a dead-but-reachable dashboard page + API~~ — **CLOSED (Aug 25).** See the Build Log's "Real Fabricated-Data Finding" entry for full detail. `Public/dashboard.html`, unreachable via any internal link but never actually exempted from the site's password gate, was reachable directly by URL by anyone who'd unlocked the site — and its `/api/get-dog-dashboard/:dog_id` route took a real `dog_id` (embedded in every real dashboard/check-in/breed-guide link this app has ever sent), pulled that dog's real check-in history, and fabricated a fake "Similar Dogs" comparison series via `Math.random()`, captioned as real "anonymized, owner-reported observations from similar dogs." Both the page and the API route are now deleted outright, verified live (`node --check` passes, server boots clean, the URL now 404s honestly instead of serving anything). **Flag this directly to the eventual lawyer review, not just via the general data-accuracy standing rule** — this is a concrete, found-and-fixed pre-launch instance of fabricated data being presented as real to a real user (via a real dog's real ID), not a hypothetical risk. Worth stating plainly in that conversation: it was found and closed before any real beta user could have encountered it, during an unrelated marketing-page accuracy pass, not through a scheduled security review — which is itself worth naming as a gap (no such review had been run) rather than treated as reassuring.
22. **Known gap, not fixed: `/checkins/:owner_id` has zero rate limiting.** Found during the resend-dashboard-link security work (Aug 26/27) that closed the timing side-channel and added anti-enumeration limiting on `/api/resend-dashboard-link` itself. `apiRateLimit` (the generic 10-req/10-sec-per-IP limiter) is wired up as `app.post('*', apiRateLimit)` — POST routes only. `/checkins/:owner_id` is a `GET` route, so it currently has no rate limiting of any kind, not even the loose generic one. Confirmed low practical severity, not a reason to leave it undocumented: `owners.id` is a genuine `uuid PRIMARY KEY DEFAULT gen_random_uuid()` (122 bits of randomness), not sequential or predictable, so brute-forcing it directly is computationally infeasible. The real residual scenario this gap matters for: if a real owner UUID were ever exposed through some *other* channel (server logs, a referrer header, an accidental screenshot), this route would do nothing to slow down repeated use of that specific leaked ID. Flagged here as defense-in-depth for a future pass, not scoped or estimated yet.
23. **Structured vet-visit-outcome data as a new tracked category** — capturing what a vet observed/confirmed during a visit as real structured, aggregable data (not just a printable guide). This is a new data category requiring the same lawyer review and data-model separation prerequisites already flagged for medication tracking (items 17/18) and adverse events (item 20), since it represents second-hand clinical data rather than pure self-report. Not started, not scoped.
24. **Aggregate free-text note theme mining — a potential third B2B revenue line, distinct from the existing structured-health-data licensing thesis.** Idea: rather than licensing raw/identifiable note content (which remains permanently excluded per the existing free-text export ban), analyze aggregate THEMES across all owner notes (e.g. "X% of senior dog owners mention bad breath concerns this month," "paw injury mentions spike in summer") and sell that pattern-level consumer-insight data to retail/consumer buyers (e.g. Chewy, Petco, Amazon) — a different customer and business motion than the current pharma/insurance/supplement predictive-modeling thesis. Real open questions before this could ever move forward, not yet resolved: needs its own explicit user consent/disclosure (the current free-text disclosure only covers security — "not reviewed or filtered" — and says nothing about aggregate theme analysis for business insights; this is a genuinely new data use, not a rewording of the existing one); re-identification risk in "aggregate" insights needs real privacy engineering, since small samples or unusual detail combinations could still be traceable back to individuals even in theme-level output; only viable at real scale — not remotely relevant until well past beta, once genuine note volume exists; and this is a distinct sales motion/buyer from the current B2B thesis, needing its own validation rather than an extension of existing pharma/insurance conversations. Status: idea captured, not scoped, not started, no legal review yet. Revisit once past beta with real data volume.
25. **Sitewide section spacing tightening** — `public-insights.html` got a scoped padding reduction (`.content-section`/`.page-hero` base padding 88px/86px reduced to 40px via page-specific modifier classes) to make its sections feel more connected. Consider applying similar tighter spacing sitewide for visual consistency across all ~10-12 pages using these shared classes. Real risk to check first: the homepage hero has precisely-tuned positioning (eyebrow position, explicit "H1 must not move" constraint, exact pixel compensation for the heading line-break) from earlier work — any change to `.page-hero`'s base padding needs to re-verify that positioning holds, not assume it does. Needs a dedicated page-by-page visual pass, not a blanket global change. Not started, not scoped.
26. **Known gap, not fixed: server-rendered pages have zero favicon links of any kind.** Found while replacing the favicon assets (Aug 28). All 13 static `Public/*.html` pages carry the same 3 `<link rel="icon">` tags plus (as of this session) a new `apple-touch-icon` tag — but every server-rendered route in `server.js` (dashboard, check-in, breed guide, Journey Summary, `/checkins/:owner_id`, `/verify`, admin, etc.) builds its HTML via template literal and includes no favicon `<link>` tags at all. A real user's browser tab shows no icon (just a generic default) on any of these pages — which is most of the actual logged-in-feeling product surface, not just marketing pages. Pre-existing since these routes were first built, unrelated to and not caused by this session's favicon swap. Not scoped or estimated — would need the same 4 link tags added to whatever shared HTML-head-building helper (or each route individually, if there isn't one) these routes use.
27. **"This Week at a Glance" card has the same evaluative-language issue already fixed elsewhere (flagged Aug 27, not yet fixed).** `describeTrendForGlance` (the dashboard's "This Week at a Glance" comparison text) still uses "improved"/"declined" — the same evaluative-language problem found and fixed the same day in the "Worth a look" health-alert card and its Journey Summary counterpart (see the Build Log's Aug 27 entry): calling a score change "improved" or "declined" is itself a judgment about what the number means, not a neutral fact. Explicitly flagged as a known follow-up in that fix's own commit message rather than bundled in, since it's a separate function/card. Not yet fixed. Should follow the same neutral pattern already applied elsewhere ("changed from X to Y" / "stayed at X") for consistency now that one instance of this issue has already been corrected.
28. **No SendGrid message-ID capture or logging anywhere in the codebase (found Aug 28, during Contact Us verification).** Every email-sending function — the two new ones built for Contact Us, and every pre-existing one (churn alerts, dashboard-link resend, etc.) — awaits `sgMail.send()` and only logs a generic success/failure line, never capturing the real message ID from the response (`response[0].headers['x-message-id']`). This makes "confirmed sent" mean "the API call didn't throw," not something independently traceable afterward. Surfaced when this session tried SendGrid's own Activity API as an independent delivery check and got a 403 (the configured key lacks that scope). Not a functional bug — no email has ever been shown to fail silently because of this — but it structurally weakens future verification passes. Low priority, not scoped: would need capturing and logging the message ID in each function, a small, mechanical, low-risk change if ever prioritized.
29. ~~Hero graphic redesign — not yet integrated~~ — **DONE (Aug 31).** The map/logo/photo-ring composite (US map in a circular frame, 5 real dog-photo positions in a ring around a center logo badge, dashed connector lines) is now live on the real homepage (`index.html`), replacing the old 6-dog hub-and-spoke design — see the Build Log's Aug 31 entries for full detail (first commit of the isolated preview + photo wiring + aspect-ratio bug fix, then the separate commit wiring it into `index.html` itself). The Aug 30 stale-render mystery was also resolved (mundane cause: no node process was actually running, and the app has no cache/build-step layer that could serve stale static content — `express.static('Public')` reads from disk on every request) — not a real bug, closed. Verified live on the real homepage at 3 widths, container and all 5 photos confirmed genuinely square at each, zero console errors, no downstream layout shift. No open items remain on this project.
30. ~~New gap found (Aug 31), not yet fixed or scoped: no real email-unsubscribe mechanism exists~~ — **FIXED (Sep 1), no open items remain.** Real, working mechanism shipped: `owners.email_opt_out` (migration run), a real `GET /unsubscribe/:owner_id` route (idempotent, doesn't expire — deliberately not the app's usual expiring magic-link pattern, since an unsubscribe link needs to stay safely re-clickable indefinitely), and a shared `buildEmailUnsubscribeFooter()` wired into both churn email templates. The gate is owner-scoped (matching the churn email's own owner-scoped combined-send design) and fails **open** on a lookup error, so a transient DB glitch can't silently and permanently block a real re-engagement email. `faqs.html`/`privacy.html` copy updated in the same commit to describe the real mechanism instead of a vague, undescribed promise — confirmed live (`grep` for "unsubscribe" in both files now returns the real link language, not the old bare promise). Verified across two sessions with a real disposable owner: dry-run trace confirmed the gating logic before the migration ran, then a full live pass after — real `200`/opted-out-confirmed response, independently re-read from Supabase (not trusted from the route's own response), the gate correctly skipping a real churn-send attempt for the opted-out owner while a control (non-opted-out) owner correctly still received one. Full detail in the Build Log's Sep 1 "A Real Email Unsubscribe Mechanism" entry. **A doc-sync gap was found and fixed Sep 2**: `All_Email_Communications.md` (created Aug 31, before this feature shipped) didn't mention the footer or the gate at all — fixed as part of this session's doc audit.
31. ~~Churn/re-engagement email had no weekly cap — could re-fire up to ~7×/missed week~~ — **FIXED (Aug 31).** Surfaced by a full per-logger min/max/cost communications trace: the dedup in `evaluateDogForChurn` only blocked a re-send within a rolling 24-hour window, not per week, so an unresolved week could re-send roughly daily for as long as it stayed missed — up to 77 churn emails across an 11-week disengaged program, against an intended one-nudge-per-week design (matching the SMS reminder cascade's own restraint, which was already correctly capped at 3/week). Fixed as a query-logic-only change (no schema/migration needed — `churn_flags.week_number` already existed and was already the match key): dedup now skips if *any* `churn_flags` row exists for that dog+week, regardless of age. Verified via a dry-run trace of the real, post-fix functions (extracted verbatim, no live sends) simulating a full 12-week program: MAX now sends exactly 11 (one per missed week, weeks 2–12), MIN remains 0. Full detail in the Build Log's Aug 31 "Full Communications Audit" entry, including a real methodology bug found in the verification harness itself (real wall-clock `Date` wasn't parameterized) worth remembering for future dry-run audits. New reference doc `All_Email_Communications.md` created alongside this fix. No open items remain on this specific finding.
32. ~~8 structured signup fields collected but never exported to Google Sheets~~ — **FIXED (Aug 31), one manual step still needed from John.** A full audit (not just the 4 fields flagged in passing during the prior session's synthetic-data work) found 8 real gaps in the Signups tab: `spayed_neutered`, `diet_type`, `treatment_category`, `pet_insurance` (previously known), plus `zip_code`, `phone`, `sms_consent`, `consent_given_at` (new). CheckIns tab confirmed to have no gap — everything structured already exports except the 8 item-level mobility/cognitive sub-scores, which are deliberately composite-only per STEP P10 Stage 1's own documented decision, left untouched. Free-text `baseline_notes`/`observation` confirmed still correctly excluded from both tabs. Fixed via one shared helper (`buildSignupSheetsExtraColumns`) used by both real write call sites (`/verify`, `/api/add-dog`), so they can't drift apart the way headers and writes have before in this project. A real bug was found only by actually checking a live write, not just that it didn't error: `phone` was silently losing its `+` prefix (Sheets parsing it as a signed number) — caught and fixed via a second real signup test, re-verified. **Manual step required: add 8 header labels to columns M–T of the live Signups sheet** — `Phone`, `Zip Code`, `Spayed/Neutered`, `Diet Type`, `Pet Insurance`, `Treatment Category`, `SMS Consent`, `Consent Given At` — same "header-creation code doesn't retroactively touch an already-existing tab" limitation as the Aug 23 "(0-10)" label change. Full detail, including the real end-to-end verification pass (3 real signups through the actual running server, edge cases: leading-zero zip codes, a 3-valued `pet_insurance` field confirmed genuinely 3-valued, a `treatment_category: ['none']` case), in the Build Log's Aug 31 "Full Structured-Field Sheets-Export Audit" entry.
33. ~~Follow-up live verification needed once `migration_add_medications.sql` runs~~ — **FULLY VERIFIED (Sep 1), no open items remain.** Migration confirmed live against Supabase first (both tables reachable, `pending_medications` column present), then the real 0→1→2-active→stopped sequence run end-to-end against live data on a disposable test dog: 0 medications correctly renders no check-in question on both surfaces (one false-positive grep match traced to an unrelated pre-existing Vet Visit Guide section and ruled out); 1 active medication correctly shows the single yes/no toggle with the right category-specific label, no which-one selector, and auto-attributes a real submitted update to the right medication; a 2nd medication correctly triggers the which-one selector, and a targeted submission was confirmed attributed to the 2nd medication specifically, not the 1st; stopping the 1st medication correctly dropped it from active-counting on the very next check-in (reverting to single-toggle behavior referencing only the 2nd) while its historical weekly-update row from step 2 stayed byte-for-byte untouched. Zero owner-identifying data confirmed against actual live row content (not just schema) — no matching column names, and the dog's real phone/email were explicitly searched for and never found anywhere in either table's data. **A follow-up pass then verified the one remaining path**: a real 2-medication baseline signup through the actual `/api/send-magic-link` → `/verify` flow, confirming `pending_medications` staged correctly on `magic_link_tokens` (both entries, every field intact) and both medications landed in the real `medications` table field-by-field matched, correctly attributed to the newly created dog's real ID. All test data across both passes cleaned up and confirmed at zero, including table-wide totals for both new tables (proving these tests were the only data either has ever held) and, for the second pass, the real `owners` row it created. Full detail in the Build Log's two Sep 1 entries ("Full Live 0→1→2→Stopped Verification" and "Baseline-Signup Staging Path Verified Live"). The medication-tracking system is now fully verified across every path it can be reached through: baseline signup, direct add, weekly update, and stop.

34. **US-residents-only signup gate — real but partial, built Sep 1. Found missing from this checklist during the Sep 2 doc-sync audit.** Two related fixes, both confirmed still live: (a) the ZIP code check tightened from a bare 5-digit regex (which couldn't actually distinguish a US ZIP from several other countries' own 5-digit postal codes) to `/^\d{5}(-\d{4})?$/` — accepts real ZIP+4, explicitly rejects anything with a letter (the specific thing that would catch a Canadian postal code), confirmed live in `/api/send-magic-link`. `terms.html`'s Eligibility section now states "Be a resident of the United States," confirmed live. (b) `sanitizePhone` rewritten to use `libphonenumber-js`'s real NANP area-code data instead of digit-count guessing, closing the specific gap Twilio's own account-level Geo Permissions structurally can't close (the US and Canada share the `+1` country code, so a Canadian `+1` number passed Geo Permissions and the old app logic equally) — a real Toronto-area-code number is now correctly rejected where it previously wasn't, confirmed live via `parsePhoneNumberFromString(phone, 'US')` + `parsed.country === 'US'`. **Neither fix is a hard technical gate** — this is deliberately an interim measure appropriate for a small, personally-vetted closed beta, not a production-grade residency check (a foreign 5-digit ZIP that happens to also be a real US-format string, or IP-based geolocation, are both still unaddressed) — real decisions on stronger gates were explicitly deferred pending real launch scale, per the Build Log's own Sep 1 entries. Also found and rotated in the same session: a real Twilio auth-token leak via the `Accounts.json` endpoint, which produced the now-permanent CLAUDE.md rule banning that endpoint. Full detail in the Build Log's two Sep 1 entries ("A Real Credential Leak..." and "Real Phone Validation...").

35. **`medications.condition_treated` converted from free text to a closed 10-value dropdown + free-text fields stopped syncing to Google Sheets — built and verified Sep 1, migration run Sep 2. Found missing from this checklist during the Sep 2 doc-sync audit.** Two related changes shipped together: (1) `condition_treated` now validates against a closed vocabulary (`arthritis_joint_pain`, `hip_elbow_dysplasia`, etc., plus an `'other'` escape valve paired with a new `condition_treated_other_detail` free-text column, itself permanently excluded from any B2B/licensing export per the standing rule) — `migration_medication_condition_dropdown.sql` confirmed run live (Sep 2, alongside the medication-event feature's own migration). (2) `dog_notes.note_text` and `mobility_checkins.observation` stopped syncing to Google Sheets in any form — the Notes tab's live sync was removed entirely (historical rows left untouched) and the CheckIns tab's "Notes" column dropped from what's written, making the project's existing "free text never reaches the B2B export" rule's scope explicit rather than enforced ad hoc per field. A third piece (new Medications/MedicationUpdates Sheets tabs) was built and verified but deliberately held back, since John wants to decide separately whether medication data belongs in Sheets at all — saved as a standalone, not-yet-applied patch at `part3_medication_sheets_tabs.diff` (untracked, confirmed still `git apply --check`-clean as of this audit). **Manual step still needed from John**: remove the "Notes" label from column J of the live CheckIns sheet's header row (code-side removal already verified; the live header text itself needs a manual edit, same recurring limitation as every prior header change in this project). Full detail in the Build Log's Sep 1 "Structured condition_treated Dropdown" entry.

36. ~~Medication-event-triggered opt-in + milestone-payoff messaging — BUILT (Sep 2), migration-gated verification still pending~~ — **FULLY VERIFIED (Sep 2, same day), no open items remain.** Leverages the already-unbounded reminder/churn/display infrastructure confirmed live the same session (no `week_number` ceiling anywhere in that pipeline — this is medication-event *awareness* layered on top, not a new "keep logging past 12 weeks" mechanism). New `medication_response_windows` table (`migration_add_medication_response_windows.sql`, run) tracks 3 real event types — a medication started after baseline, stopped at any point, or changed via a real weekly-update `changed_switched` answer — each creating one row via a new shared `createMedicationResponseWindow()`. In-app only (dashboard banner + the standalone check-in confirmation screen), matching the existing Aug 18 "health journey, not a dataset contribution" rule already governing `getStreakMilestoneMessage` — no new SMS/email send type. Domain mapping (`getMedicationEventDomain`) uses `condition_treated` first (more direct signal — "digestive_gi" unambiguously means appetite) with `category` as fallback and `energy` as a final catch-all, a deliberate refinement of the task's own literal category-only example, flagged rather than applied silently. Milestone-payoff fires every 4-week multiple after the event (4, 8, 12, 16...), reusing `getStreakMilestoneMessage`'s own never-go-silent-past-12 extension pattern rather than 3 fixed checkpoints. Trend copy (`describeMedicationEventTrend`) deliberately reuses `describeJourneyTrend`'s neutral up/down/steady arithmetic and framing, not `generatePostLogInsight`'s inherently evaluative "better/worse" language named in the literal spec — flagged as a correction, since the neutral-language requirement and that specific reuse target were in direct tension. A real gap was found and closed before building: `POST /api/medications` and `POST /api/medications/:id/stop` have zero live frontend callers (confirmed via a full grep of `Public/`), and the literal 3-trigger-point spec would have left the one real, reachable "stop a medication" UI action (the weekly check-in's chip flow) never creating a window at all. Fixed by (a) extending the weekly-update handler to fire on `stopped` as well as `changed_switched`, and (b) adding a persistent dashboard banner for any *pending* (unanswered) window regardless of which endpoint created it — the real, durable surface that makes the two headless endpoints' prompts actually visible to an owner. General streak milestones and medication-event milestones are shown together, never suppressed against each other — different information, already separate UI blocks, no real redundancy. **Full live verification, once both this migration and `migration_medication_condition_dropdown.sql` (Sept 1) were run**: real `POST /api/medications` (started), real weekly-update chip submissions for both `stopped` and `changed_switched`, on two disposable test dogs. Dog A: opted in to a started event, dashboard pending-banner confirmed rendering correctly before responding, then advanced 4 weeks and confirmed a real computed trend (`"Mobility has moved down from 6/10 to 2/10 since then (-4)."`, hand-verified against known baseline/submitted values) on both the confirmation screen and the dashboard. Dog B: two medications, one stopped-and-declined, one changed-and-opted-in — at the declined window's own +4 week, correctly zero milestone; at the opted-in window's +4 week, correctly exactly one milestone with the right real trend, the declined window still correctly absent — proving opt-in and opt-out gate independently and correctly on the same dog. **One real bug found and fixed during this pass**: `computeDueMedicationMilestones`'s row-filtering logic unconditionally discarded the current week's own check-in row before optionally replacing it with a fresher in-memory snapshot — correct for the `/api/checkin-senior` caller (which always supplies a replacement, to avoid racing its own not-yet-committed insert), silently wrong for the `GET /dashboard/:dog_id` caller (which supplies nothing and relies purely on the query), so the dashboard banner showed a false "stayed at X" instead of the real trend on every view. Fixed to only filter-and-replace when a replacement is actually available; re-verified live immediately after, dashboard and confirmation screen now show byte-identical trend text. All test data cleaned up, confirmed at 0 per dog and 0 table-wide for `medication_response_windows`. Full detail in the Build Log's two Sep 2 entries.

37. ~~**PRE-BETA BLOCKER, real and urgent: `GET /api/get-all-dogs` was an unauthenticated full-database dump against the live `senior_dogs` table**~~ — **FIXED (Sep 2), no open items remain.** Found in passing during the link-revocation Phase 1 audit: this route (unchanged since the Aug 14 bulk-import commit, `9cd0be7` — the same commit that produced the already-removed dead `dashboard.html`/`get-dog-dashboard` pair) returned every dog's `id, dog_name, breed, age, gender, baseline_mobility_score, created_at` with no pagination, no cap, and no dedicated authentication of any kind — the only thing in front of it was the site-wide `SITE_PASSWORD` wall, a temporary pre-launch mechanism this project's own checklist already has flagged for removal (item 2). Unlike the already-known dead legacy-schema routes, this one queried a table that's very much alive — real, current data, not an error. Confirmed genuinely dead in the frontend (zero callers anywhere in `Public/`, checked twice) and confirmed via `git log -S` this was a leftover internal debugging tool (its own original comment: `// GET ALL DOGS (for testing/debugging)`), never touched again after that one commit. Not a contact-info leak — no join against `owners`, no `phone`/`email`/`zip_code` selected — but a real exposure of identifying-adjacent structured data for every dog, currently inert only because `senior_dogs` is at 0 rows today, not because the bug isn't real. **Fixed by outright deletion**, matching the same reasoning as the Aug 25 dead-dashboard removal. Verified: `node --check` passes, a clean local server boots with zero errors, the URL now correctly 404s. Full detail in the Build Log's Sep 2 (continued) "Real Pre-Beta Blocker Found and Closed" entry.

38. ~~Sibling routes checked, none fixed — flagged for a separate discussion, not touched~~ — **RESOLVED (Sep 2), all 4 accounted for.** Full investigation of each (complete handler read, read-only-vs-side-effects, live-schema-vs-dead, auth check) done before any removal — see the Build Log's Sep 2 entries for the exact reasoning per route. **Removed:** `POST /api/test-email` — the single most urgent finding of this whole audit, more so than `get-all-dogs` itself: confirmed to send a real, live, unauthenticated email via the project's real SendGrid account to **any address supplied in the request**, with no ownership check and no dog-existence validation — exploitable today regardless of beta data volume, a genuine harassment/impersonation/quota-abuse vector, not just a data leak. `POST /api/test-churn-detection` — confirmed safe-by-design before removing (verified `evaluateDogForChurn`'s `sendSmsReminders` defaults false and is never overridden here, and `sendChurnAlertsForOwnerGroup`'s email resolution can never fall back to a real owner at this call site even under misconfiguration), removed anyway since a real internal QA tool has no reason to be reachable with zero auth once beta data exists. `POST /api/age-bella-data` / `POST /api/clear-bella-alert` — confirmed inert (hardcoded to one fixed, RFC-4122-example-pattern UUID, confirmed live at 0 matching rows in every table it touches, not attacker-redirectable to any other dog), removed anyway on the principle that unauthenticated write/delete capability has no reason to sit in a production file regardless of current blast radius. Two stale comments elsewhere in the file that referenced the removed routes were also updated. Verified: fresh grep confirms zero remaining references (except one intentional historical note explaining the removal), `node --check` passes, clean server boot, all 4 removed routes now 404 live. **`GET /api/governance/stats` — deliberately left as-is, not a security/cleanup item.** John's call: this is a genuinely good idea (a real public transparency dashboard — total loggers, SMS opt-in rate, check-in volume — as a trust-building feature), just not needed yet, since there's no real beta data to show. Reframed as a future feature idea, not a pre-beta blocker — see item 39. The route itself stays exactly as-is, untouched, low severity (read-only, aggregate-only), same incidental `SITE_PASSWORD` protection as before.

39. **Public transparency dashboard (`GET /api/governance/stats`) — real feature idea, deliberately deferred to post-beta.** Surfaced during the Sep 2 sibling-route audit: this endpoint already exists, live, and already computes real aggregate metrics from the current schema (`senior_dogs`/`mobility_checkins` — total loggers, SMS opt-in rate, total check-in volume). Its presumed original frontend, `governance.html`, is currently just a 12-line redirect stub to `privacy.html`, not a real page. John's decision (Sep 2): the underlying idea — a real, public, numbers-only trust page — is worth building, but only once there's actual beta volume worth showing; a transparency dashboard reporting "0 loggers" undermines the trust-building point it's meant to make. Revisit once real beta signups exist. Not scoped, not estimated, not building yet — the backend groundwork (the stats endpoint itself) is already done whenever this gets picked up.

40. ~~Link revocation & access-token system — every dog/owner link made revocable~~ — **DONE (Sep 3), live-verified in production, no open items remain.** Closes a real structural gap true since the app's first dashboard route: a raw `dog_id`/`owner_id` UUID, by itself, was permanent, unrevocable access — most consequentially, `POST /api/add-dog` let anyone holding or guessing a real `owner_id` attach a fabricated dog to a real account. Built: `owners.access_token` (real column, `UNIQUE`, migration run); every dog/owner-scoped route (12 total — 4 read-only, 8 write-capable) now authorizes on token **or** owner-session cookie, never a bare ID; every mailed/emailed link (8 real construction sites, more than the originally-scoped 6) now embeds the live token, including every internal same-page link/fetch those routes themselves render, via one shared `withToken()` helper, so a token-only visitor never hits a dead end; a new public recovery flow (`/recover.html`, phone-only lookup, a separate short-lived single-use token, dual-channel delivery); a new dashboard "Regenerate My Link" button, deliberately session-cookie-gated only, not token-eligible, so a leaked link alone can never rotate itself out from under the real owner. A real gap found in review before anything was committed — the owner-session cookie was unsigned plaintext, forgeable by anyone who'd ever seen any link for that owner, meaning "Regenerate" didn't actually revoke a lingering session on another device — fixed with HMAC signing (`crypto.timingSafeEqual`, per this project's own prior Aug 29 timing-side-channel finding) and live-proven: the exact bug scenario (old session cookie still working after regeneration) directly reproduced, then directly confirmed fixed. All 7 required checks passed against the real production server, not just localhost — booted clean with the new `SESSION_SIGNING_SECRET`, a genuine token-based link worked, a session cookie worked, regenerating correctly invalidated both the old token and the old session cookie in the same cycle, the recovery flow sent a real Twilio send, test data cleaned up and confirmed at 0. Full design rationale and every verification pass in `Link_Revocation_Build.md`. Two items deliberately left open — see items 43–44 below.

41. ~~Privacy Policy / Terms of Service — full content rewrite, published live~~ — **DONE (Sep 3), live-verified in production, no open items remain.** Replaced both pages' content with the reviewed drafts (DRAFT banner and internal review notes stripped), governing law kept as State of Texas per confirmed-correct direction, `privacy.html` rebuilt onto `terms.html`'s existing dense numbered-section template rather than the reverse. Investigation first caught a real stale premise in the drafts themselves — they described persisting a signup consent record as still-needed future work, when `consent_given_at`/`consent_policy_version` were already live and correct (shipped Sep 2, see item 35 and the Build Log) — corrected before any content shipped, avoiding rebuilding an already-working mechanism. Signup consent checkbox strengthened on both `baseline-health-journey.html` and `add-dog.html` to name and link both real pages (`target="_blank"`, so checking a policy mid-signup doesn't discard the form). **A real bug found via an actual manual click, not a diff review**: the strengthened checkbox's own link was correct, but two separate, pre-existing links nearby ("See our Terms & Privacy" near the SMS field, "Learn more." near the submit button) pointed at `trust-and-data.html` with no `target` attribute — either one discarded the whole in-progress signup form on click. Both fixed. All content and link fixes confirmed live on production via direct `curl` checks (section counts, date strings, link destinations), not assumed from a successful push. Full detail in `Legal_Pages_Publish_Build.md`.

42. ~~FAQ verbiage pass against the newly-live legal pages~~ — **DONE (Sep 3), live-verified in production, no open items remain.** With real, specific legal-page content now live (item 41), a systematic cross-check of every `faqs.html` entry touching data sharing, consent, retention, account access, opt-out, medical disclaimers, and jurisdiction surfaced 6 real discrepancies across 8 answer blocks — none outright false, all materially vaguer or softer than the real, live commitment: `#f6`'s "not used for intelligence at this time" language understated a permanent, architecturally-enforced exclusion and listed a far narrower collected-field set than actually disclosed; the `#f5`/`#f7`/`#f18` cluster never named which categories of company (pharma, research, supplement, food, insurance, product manufacturer, retail) license the de-identified data, despite the real policy stating this plainly as the core business model; `#f21` implied participation was separable from licensing consent when the real policy bundles them; `#f8` never addressed that de-identified data is retained indefinitely; `#f11` omitted the passwordless, link-as-password account model; `#f15` referenced "our Privacy Policy" as plain text rather than a real link. All 6 rewritten to match, live-verified through the page's real accordion toggle (all 8 answers, open and close, zero console errors) and a real click test on the new link, then confirmed live on production. Full detail in `Legal_Pages_Publish_Build.md`'s Phase 3 section.

43. ~~`npm audit`: 3 moderate `qs` vulnerabilities via `body-parser` → `express`, no non-breaking fix available~~ — **FIXED (Sep 4) via a targeted `package.json` `overrides` pin, no open items remain.** Found during the link-revocation build's pre-deploy security sweep. `npm audit fix` (run, not forced) made zero changes — the patched `qs` version requires `body-parser@2.x`, which requires `express@5.x`, a major framework upgrade judged out of scope for a moderate-severity, pre-beta fix. Resolved instead with `"overrides": { "qs": "6.16.0" }` in `package.json`, keeping Express 4/`body-parser` 1.x otherwise completely untouched — confirmed live: `node_modules/qs/package.json` reports `6.16.0`, `npm audit` now reports 0 vulnerabilities. Verified as real, not just "installed cleanly," since this overrides `body-parser`'s own declared compatibility range: started the real dev server and exercised both code paths that actually route through `qs` — a GET request with bracket/array-notation query params (`?a[b]=1&arr[]=x&arr[]=y`, the same shape the array-limit-bypass advisory concerns) and POST requests with both a JSON body and a `application/x-www-form-urlencoded` body (`extended: true`, which is what pulls `qs` in for body-parsing) — all returned correct, non-error responses, zero server errors in the logs. Additionally called the pinned `qs` module directly and confirmed it parses the same bracket-notation string into the exact expected nested-object/array shape, not just "didn't throw." `node --check server.js` passes; a clean local server boot confirmed with zero errors. **The Express 5 upgrade itself remains a deliberately separate, deferred future decision** — this fix only closes the `qs` advisory, it does not revisit whether/when to move off Express 4. Full detail in `Link_Revocation_Build.md`'s Backlog section.

44. ~~`authorizeOwnerScope`'s `access_token` comparison uses `===`, not `crypto.timingSafeEqual`~~ — **FIXED (Sep 4), no open items remain.** Originally deliberately deprioritized when found (same *class* of non-constant-time string comparison as the session-cookie signature bug fixed in item 40, found while implementing that exact fix — `access_token` is a 36-character random UUID already transmitted in plaintext by design, so its threat model is "hard to guess," not "resistant to being observed," materially lower priority than the session-signature forgery gap that prompted the original fix). Closed anyway rather than left open indefinitely: replaced the plain `===` with `crypto.timingSafeEqual`, matching the session-cookie fix's exact pattern — a length guard first (`timingSafeEqual` throws on unequal-length buffers rather than returning `false`), only calling it once lengths match. Live-verified against the real running server on a disposable test owner/dog: correct token authorizes (`200`); a same-length wrong token is rejected (`403`); both shorter- and longer-than-real wrong-length tokens are rejected (`403`, confirmed via server logs that this was the length guard catching it, not an unhandled throw); a missing/empty token is rejected (`403`). A file-wide grep confirmed this was the only `access_token`/`realToken`/`providedToken` `===` comparison anywhere in `server.js`. Test data cleaned up, confirmed at 0. Full detail in `Link_Revocation_Build.md`'s Backlog section and the Build Log's Sep 4 entry.

45. **`part3_medication_sheets_tabs.diff` — a real, working patch sitting untracked and unapplied, pending John's decision. `git apply --check` now fails — flagged Sep 4, not yet fixed.** Built and verified Sep 1 (see item 35): new Medications/MedicationUpdates Google Sheets tabs, wired into all 4 real medication call sites, with the standing free-text-export rule already correctly enforced (the `note`/`condition_treated_other_detail` fields never even reach the append function's parameters). Deliberately held back before commit because John wants to decide separately whether medication data belongs in Google Sheets at all, not because anything about the patch itself was unfinished. **Re-checked Sep 4 as part of a full doc-currency pass: `git apply --check part3_medication_sheets_tabs.diff` now fails** (`patch does not apply` against `server.js`) — the surrounding code has moved enough since Sep 1/2 (age-field rewrite, `authorizeOwnerScope` fix, and everything else shipped since) that the diff's line-offset context no longer matches. Still real, working code as originally written and verified — this is bit-rot in the patch file's applicability, not a defect in the feature itself. Real open item, now with an added wrinkle: either apply it (a decision to ship, which will need the diff regenerated/rebased against current `server.js` first, not a clean `git apply`) or delete it (a decision not to) — it shouldn't sit untracked and silently decaying indefinitely.

46. ~~`docs/LEGAL-DRAFTS/Privacy_Terms_Rewrite_Outline (1).md` — untracked, now superseded, no decision made on its disposition~~ — **RESOLVED (Sep 4), no open items remain.** Renamed to `docs/LEGAL-DRAFTS/Legal_Pages_Rewrite_Process_Notes.md` with a short archival note prepended above the original title, marking every 🔴-flagged open item as resolved in the final shipped drafts and pointing to `Legal_Pages_Publish_Build.md` for the full history. Since the whole `docs/LEGAL-DRAFTS/` directory had never been committed, this landed as a plain file addition rather than a detected `git mv` rename — no prior git history existed for a rename to actually preserve. Committed (`6bd01d9`) and pushed. **The other two LEGAL-DRAFTS files (`Privacy_Policy_Draft.md`/`Terms_of_Service_Draft.md`) remain untracked** — same open question as before, not addressed by this item, not otherwise flagged elsewhere on this list.

47. **Begin recruiting for the beta — still not started as of Sep 4, 2026.** Restated from item 6, since it's the single largest genuinely-open item on this whole list and easy to lose among the security/legal work above: 8 real strangers matching the actual target persona, plus 2–3 close friends, per the locked Aug 20 beta-structure decision. The site remains `SITE_PASSWORD`-locked (item 2) and has zero real users. Nothing about the security, legal-pages, or signup-form work done since Sep 3 changes this — if anything, all of it was prerequisite work worth having in place *before* real strangers start signing up (a real security sweep, live legal pages, a signup form that actually supports the full range of real dogs), not blockers to recruiting itself. No technical blocker remains that this checklist is aware of; this is a real decision and a real outreach effort, not a build task.

48. **Signup survey fixes: age-under-1-year support, notes-field disclosure, treatment/medication merge — both signup forms, live in production (Sep 4).** Three related fixes to `baseline-health-journey.html` and `add-dog.html`, shipped as three separately-reviewed commits (`d392ea1`, `42854fd`, `7abe58b`):
    - **Age under 1 year.** The age field forced a whole-number "years" input with a hard minimum of 1 — no way, client or server, to register a puppy under 12 months old, contradicting the Aug 20 decision to keep data collection open to all ages. Replaced with two fields (Years, Months 0-11) on both signup forms; new `getDogAgeInYears()`/`formatDogAge()` helpers used by all `isSeniorForBreed()` call sites and every age-display surface (dashboard header, dashboard Health Journey section, Journey Summary modal). New `senior_dogs.age_months`/`magic_link_tokens.age_months` columns (`migration_add_dog_age_months.sql`, run). **Two previously-unknown legacy CHECK constraints found and fixed along the way, the same class of surprise as the earlier `mobility_check` incident (STEP P10, Aug 23)**: one on `senior_dogs.age` (worked around by the new two-field design), and a second, separate `age_check` constraint on `magic_link_tokens.age` found only via a real end-to-end test after the first migration ran — a genuine 0-years/8-months submission was rejected by a stale "age ≥ 1" rule nothing in this change had touched (`migration_drop_legacy_age_check.sql`, written, then confirmed run by John, then re-verified live via a real puppy signup through the main `baseline-health-journey.html` → `/api/send-magic-link` → `/verify` path).
    - **Notes-field disclosure.** A short in-context disclosure ("This isn't reviewed or filtered — avoid including anything you wouldn't want stored as plain text.") added directly under the observations textarea on both forms, plus the matching disclosure for the medication "other" free-text field. Confirmed missing during investigation — called for by the Aug 20 decision but never actually built on either form.
    - **Treatment/medication merge.** The 7 treatment-type checkboxes and the separate, always-visible Medications & Supplements section stored the same fact twice with no reconciliation. Checking a non-"none" checkbox now reveals an inline entry section locked to that category (a hidden `.med-category` input, category implied by which checkbox triggered it); supports multiple entries via "+ Add another"; unchecking a box with a filled entry confirms before discarding it. `treatment_category` now derives client-side at submit time from which categories have a filled entry — confirmed via reading every real consumer first that this required zero server-side changes.

    **Verified both locally and against production.** Locally: a real 0-years/8-months puppy submitted through the actual running server's `baseline-health-journey.html` (real form fill, real click, real `POST /api/send-magic-link` → `/verify`) landed as a real `senior_dogs` row (`age: 0, age_months: 8`), with `formatDogAge()`/`isSeniorForBreed()` confirmed correct on the dashboard, Journey Summary, and breed guide (correct locked-until-week-2 state, no crash). The 3 commits were split out of one combined working-tree diff via the same revert/reapply commit-surgery pattern used in the link-revocation build, verified byte-for-byte identical to the pre-surgery ground truth before pushing. After deploy, a **structure-only check against production** (no real signup submitted, per explicit instruction) confirmed both forms render the age fields, notes disclosure, and all 6 treatment-category blocks correctly, with the old single age field and old medication section confirmed gone — done by unlocking the live `SITE_PASSWORD` gate via a one-way hash of the real password (John ran the unlock request locally and shared back only the resulting cookie value, never the raw password), validated against `server.js`'s own cookie-name/hash-shape before use. Full detail in the Build Log's Sep 4 entry.
