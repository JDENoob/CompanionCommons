...
10. **Multi-dog owner project (see `Multi_Dog_Signup_Build.md` for full detail)** — Stages 1-4 complete (owner entity, `owners` table, signup flow rewritten with returning-owner detection, and message consolidation — combined churn emails, combined SMS reminders via a new `/checkins/:owner_id` page). **Stage 4 has one migration (`migration_add_sms_queue_owner_id.sql`) still pending in Supabase — run it before/with this code's deploy, or the SMS-sending cron will silently stop sending.** Stage 5 (owner session + dashboard dog-switcher) not yet started.
