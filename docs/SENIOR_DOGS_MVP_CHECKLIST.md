...
11. **Chart Cognitive and Weight in the dashboard, and add Cognitive to the Baseline Score box** — pre-existing gap, noticed during STEP P10 investigation (Aug 23), explicitly NOT part of P10's scope. **Cognitive resolved (Aug 23):** Baseline Score box now has a 5th Cognitive mini-column (matching Mobility/Energy/Appetite/Weight exactly, same `?? '—'` handling), the "Exact values by week" mobile mini-table has a matching Cognitive row, the Chart.js line chart has a 4th Cognitive line (distinct color/star markers/dash-dot pattern, null on non-cadence weeks so Chart.js's default `spanGaps:false` leaves genuine gaps rather than a misleading interpolation), and the Journey Summary's Weekly log table got a Weight column too (value + "lb", em-dash when not collected that week). All verified live against Winston's real 11-week data (has real cognitive values on weeks 4/8/12 and real weight). **Weight-on-chart still explicitly open** — different unit (lbs) and different scale than the chart's existing 0-10 axis, so it isn't a simple line addition; options (secondary y-axis vs. a small separate weight-only chart vs. something else) reported back for John's decision, not built.
