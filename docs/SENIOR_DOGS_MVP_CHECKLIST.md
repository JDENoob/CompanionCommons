...
29. **In progress, not yet integrated: hero graphic redesign (Aug 30).** A new map/logo/photo-ring composite (US map in a circular frame, 5 dog-photo positions in a ring around a center logo badge, dashed connector lines) built and mostly verified — but only in an isolated preview page (`Public/assets/images/hero graphics/preview.html`), not wired into the real `index.html` hero section. See the Build Log's Aug 30 entry for full detail. Real photos are currently pulled back out (empty ringed circles) while John recrops the source images. **Open issue, not yet diagnosed:** `localhost:3000` was showing a stale version of the graphic at the end of the session despite the background/logo fixes being confirmed correct via direct render checks earlier the same session — cause unknown (server restart, browser cache, wrong URL, and a file-save issue are candidates, none confirmed). Verify directly next session before doing anything else with this.
