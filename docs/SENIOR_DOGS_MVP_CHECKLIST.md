...
30. **New gap found (Aug 31), not yet fixed or scoped: no real email-unsubscribe mechanism exists.** Both `faqs.html` and `privacy.html` promise a user can "unsubscribe from emails" (FAQ: *"Reply STOP to any SMS, unsubscribe from emails, or contact us directly..."*), but nothing in the codebase actually implements this for the email channel — no SendGrid unsubscribe group, no `List-Unsubscribe` header on any outbound email, no in-app opt-out endpoint. SMS opt-out is genuinely real and enforced (Twilio's "Reply STOP" works at the carrier level); the email half of that same sentence is currently just a promise with nothing behind it. Surfaced while verifying the new "Stop logging anytime — no obligation, no penalty, ever." copy on `trust-and-data.html` (commit `e834917`) — that specific claim checked out (no account/login/inactivity logic exists anywhere, so a user genuinely can just stop responding with zero consequence), but it led directly to finding this adjacent, separate promise that doesn't hold up the same way. Not scoped or estimated — would need either a real SendGrid suppression-group integration or a minimal in-app opt-out endpoint plus updating every outbound email to check it.
