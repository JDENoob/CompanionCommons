...
33. **Follow-up live verification needed once `migration_add_medications.sql` runs (Sep 1).** Item 17's medication-tracking system was built and had every validation path and endpoint's reachability verified live, but the actual `medications`/`medication_weekly_updates` tables don't exist yet (migration written, not run, per standing rule). Once John runs it, a real 0→1→2-active→stopped sequence needs to be exercised for the first time against live data: confirm the weekly check-in question is absent at 0 active medications, the single-toggle auto-attributes correctly at 1, the "which one?" selector appears and attributes correctly at 2+, and marking one stopped both drops it from future active-counting and leaves its historical `medication_weekly_updates` rows intact. Also confirm a real baseline signup (through `/api/send-magic-link` → `/verify`) correctly carries staged `pending_medications` through to real `medications` rows. Same two-session pattern already used for every other migration-gated feature this project has shipped.
