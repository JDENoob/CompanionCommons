...
13. **Pet food/supplement/toy recall alerts (near-term discussion, not scoped)** — pull active recall information for pet food, supplements, and toys, surface relevant alerts on affected dogs' dashboards, with an opt-in mechanism specifically for recall notifications. Real open questions to resolve before scoping: no recall feed is currently wired in (would need a real source like FDA pet food recall data or CPSC for toys); the signup form only collects `diet_type` as a broad category, not specific brand/product names, and toys/supplements aren't tracked at all today, so meaningfully matching a recall to a specific dog would need new data collection; broad unmatched alerts vs. personalized matched alerts are very different in UX and data needs; opt-in should likely be its own consent dimension, separate from the existing `contact_preference` used for check-in reminders; worth eventual legal review given real duty-of-care questions around surfacing (or failing to surface) a safety recall. Not building, not estimating.
14. **Breed-matching coverage gap (near-term discussion, not scoped)** — found during the weight-chart build: breed is a free-text field at signup (no dropdown, no autocomplete), and `getBreedGuide()` matches only via exact string after trim/lowercase against a fixed list of ~30 curated breeds. Anything that doesn't exact-match (a typo, "Lab" instead of "Labrador Retriever", "Golden retriever mix", or a real breed genuinely outside the curated list) silently falls through to the generic fallback, which has no numeric weight range at all. This affects TWO existing features silently, not something new: `isSeniorForBreed` (senior-by-breed flag) has had this same limitation the whole time, and the new `isOverweightForBreed` inherits it. No error surfaces anywhere when this happens — a real, hard-to-estimate share of beta dogs could get zero benefit from either flag with nothing telling anyone it happened. Real design options to weigh later: constrain to a dropdown/autocomplete, add fuzzy/partial matching, expand the curated breed list, some combination. Not scoped, not estimated, not building yet.
