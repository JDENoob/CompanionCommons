...
2. **Fix the three remaining real gaps from the Aug 20 privacy audit**: persist a real consent record, remove or fix the orphaned `users` table, fix the stored-XSS gap in the Journey Summary's note rendering
3. **Decide when to remove `SITE_PASSWORD`** — still genuinely just a decision
4. **STEP P1C (Comparative Feedback)** — still blocked until real breed-cohort data exists
5. **STEP P8 (full end-to-end testing)** — one clean deliberate pass; test data is now fully clean, a good moment to run this
6. **Begin recruiting for the beta** — 8 real strangers matching the actual target persona, plus 2-3 close friends
7. **Begin scoping the data-model separation work** (identifiable vs. de-identifiable fields) — flagged as a real prerequisite before any future drug-name-level data collection
8. Minor cleanup, lower priority: remove the confirmed-dead duplicate reminder system (`/api/checkin`, `survey_weekly_checkins`), fix the cosmetic churn-cron startup log message ("1 minute for testing" when it's actually 60 minutes)
9. **Run `git log --oneline` as a sanity check** — a Stage 2 migration file (`migration_add_owners_table.sql`) was found sitting fully run/verified against Supabase but never committed to git for an entire session before being caught and bundled into the Stage 3 commit. Worth a quick sweep to confirm nothing else (from this project or earlier work) is similarly run-but-uncommitted.
10. **Multi-dog owner project (see `Multi_Dog_Signup_Build.md` for full detail)** — all 5 stages complete: owner entity, `owners` table, signup flow rewritten with returning-owner detection, message consolidation (combined churn emails, combined SMS reminders via `/checkins/:owner_id`, migration run and live-verified), and an additive-only owner-session dashboard dog-switcher (`/dashboard/:dog_id` renders identically for anyone with no session — vets/family sharing links unaffected — and only adds a switcher when a session cookie matches the dog's own owner, verified adversarially against a cross-owner mismatch). A real unescaped-`dog_name` gap found during Stage 5's own build was fixed on the spot (`buildDogSwitcherHtml`). No open items on this project.
11. **Site-wide audit needed: unescaped user-controlled fields rendered into HTML without `escapeHtml()`** — this is now the third confirmed instance of the same gap (Journey Summary notes, flagged in item 2 above; the Stage 5 dog-switcher tab, fixed on sight; and the pre-existing, still-unfixed dashboard `<h1>${dog.dog_name}'s Mobility Dashboard</h1>` line, found while fixing the switcher but left out of scope for that change). Each was found independently, not by a deliberate sweep — worth a real, dedicated audit of every `${...}` interpolation of `dog_name`/breed/notes/or any other user-supplied field across `server.js`. See `CompanionCommons_Build_Log.md`'s Still Open list for full detail.
