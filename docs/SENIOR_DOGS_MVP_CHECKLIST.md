...
10. **Multi-dog owner project (see `Multi_Dog_Signup_Build.md` for full detail)** — Stages 1-3 complete and shipped (owner entity investigated, `owners` table built, signup flow rewritten with returning-owner detection). **Stage 4 (message consolidation — combining reminders/churn emails per owner instead of per dog) not yet started.** Stage 5 (owner session + dashboard dog-switcher) also not yet started.
