...
### `npm audit`: 3 moderate `qs` vulnerabilities via `body-parser` → `express`, no non-breaking fix available
...
**Left open, not silently dropped. Two real remediation paths, neither taken:**
- **Force the Express 5 / body-parser 2 upgrade** (`npm audit fix --force`) — the complete fix, but a real breaking-change surface across every route in the app; needs a full regression pass before it's safe to run, not something to do casually pre-beta.
- **A targeted `overrides` entry in `package.json`** pinning `qs` to `6.16.0` directly while keeping Express 4 / body-parser 1.x otherwise unchanged — lower blast radius, but overrides body-parser's own declared compatibility range and would still need its own verification pass (confirm nothing in this app's actual `qs` usage — query-string parsing via Express's own body-parser integration — behaves differently under 6.16.0).
- **How to apply:** revisit either path deliberately, as its own scoped piece of work, not bundled into a future unrelated change. Moderate severity, not urgent enough to block this build's deploy on its own.
