...
## September 2, 2026 (continued) — Policy-Version Tracking, a Rewritten Project Overview, and a Doc-Sync Pass

Three smaller, related pieces of work closing out the day's security-and-hygiene push.

### Consent policy-version tracking added alongside the existing timestamp

`consent_given_at` (added Aug 22) already captured a real, server-side "when" for consent — confirmed live and correctly wired at both real signup paths during this investigation. What was still missing: "which version of Terms/Privacy was actually being agreed to," a real gap the moment either legal page is next substantively revised. Added `consent_policy_version` to the same two tables (`magic_link_tokens`, `senior_dogs`), following the identical staging pattern already established for `consent_given_at`. The value is a manually-maintained constant, `CURRENT_CONSENT_POLICY_VERSION`, set to the date of the most recent commit that actually changed Terms/Privacy content — not today's date — with a standing requirement flagged directly in the constant's own comment: bump it by hand whenever that content next changes. Wired into all 3 real write points: `/api/send-magic-link` (staging), `/verify` (copied from the token), `/api/add-dog` (set directly, no token to carry it through on that path). Migration written, not run at the time — **confirmed run and live during this session's later Legal Pages Publish investigation** (Sep 3), which queried the live schema directly and found `consent_given_at`/`consent_policy_version` present and populated on both tables, contradicting a since-corrected stale assumption in the legal-drafts outline that this record still needed to be built (see below).

### Project Overview document rewritten

`CompanionCommons_Project_Overview_1.md` replaces the Aug 12 original, which was substantially out of date and contained a since-reversed business-model contradiction (affiliate marketing was listed as Priority 1; the real, current position is that CompanionCommons will never sell or promote a product, or use a dog's health data to target advertising). The rewrite covers current state, deliberately-not-yet-built items and their real blockers, open decisions, data governance rules, competitive positioning, tech stack, and standing operating rules — including the first written statement of this project's own security-sweep cadence: a lightweight credential-exposure check at the end of every session before pushing, and a full sweep (credential inventory, RLS check, dependency audit, endpoint auth review) before any real go-live event or major deploy. That cadence, established after three real credential-exposure incidents across this project's history, is the exact template this session's own lightweight checks (run twice, both clean) and the link-revocation build's full pre-deploy sweep both followed.

### A real doc-sync pass, closing the same pattern of gap it was named after

Full audit found the same pattern already caught once before with the email-unsubscribe feature repeating: real, shipped, verified work with no corresponding checklist item, and one doc actively describing a now-superseded state. Fixed: `CLAUDE.md`'s hero-graphic aspect-ratio rule only named the isolated preview page, not the live production implementation (`.community-visual` in `styles.css`/`index.html`) it was wired into on Aug 31 — extended to cover both, plus a pointer past the "Current in-progress work" section's Aug 24 stopping point to the checklist and Build Log as the real current source of truth. `All_Email_Communications.md` (created Aug 31, before the Sep 1 email-unsubscribe feature shipped) didn't mention the real footer link or the real `email_opt_out` gate anywhere — added both, plus a MIN/MAX note for the opted-out case. `SENIOR_DOGS_MVP_CHECKLIST.md` item 30 said "not scoped" for a feature that was actually built and fully verified Sep 1 — fixed; two real Sep 1 features had no checklist item at all (the US-residents-only signup gate, and the `condition_treated` dropdown + Sheets free-text-export fix) — added as items 34–35, renumbering the medication-event feature to 36 to keep sequence. `All_SMS_Communications.md` and this Build Log were both independently confirmed accurate against live code and commit history — no changes needed to either at the time.

**Supporting services/tools:** `server.js`, `migrations/migration_add_consent_policy_version.sql`, `CompanionCommons_Project_Overview_1.md`, `CLAUDE.md`, `All_Email_Communications.md`, `SENIOR_DOGS_MVP_CHECKLIST.md`.

---

## September 3, 2026 — Link Revocation & Access-Token System: Every Dog/Owner Link Made Revocable

Full design rationale, live-schema findings, and every verification pass live in `Link_Revocation_Build.md` — not duplicated here in full. This closes a real, structural gap that had been true since the app's very first dashboard route: possession of a raw `dog_id`/`owner_id` UUID was, by itself, full and permanent access, with no way to ever revoke a link once it went out.

### Phase 1 — audit, no code touched

Full inventory of every place a dashboard/check-in/dog-specific link is constructed (6 real mailed/emailed sites, plus a 7th and 8th found during the actual build — the unsubscribe footer and the resend-dashboard-link flow) and every route granting access on a bare ID (4 read-only, 8 write-capable). The single most consequential finding: `POST /api/add-dog` let anyone holding or guessing a real `owner_id` attach a brand-new, fabricated dog to a real owner's account — not "view data you shouldn't," but "write fabricated records into a real account." Live schema confirmed directly from Supabase (not inferred): no `access_token` column existed anywhere yet, and `owners.email` — unlike `owners.phone`, which already carries a real `UNIQUE` constraint — has no uniqueness constraint at all, confirmed both from the migration source and empirically (two disposable rows with the identical email both inserted successfully). Recorded as a real, named decision for Phase 2 rather than resolved silently: the future recovery flow's account lookup would be phone-only, matching the precedent `/api/resend-dashboard-link` already set.

### Phase 2 — schema, plus a standalone hardening fix found along the way

`GET /unsubscribe/:owner_id` was unsubscribing a real owner from all future email on a bare `GET` — no confirmation, meaning a link-prefetching email client or scanner could trigger it without a human ever clicking anything. Split into a read-only `GET` (renders a confirmation form, or the terminal "unsubscribed" page if already opted out) and a new `POST` that carries the actual mutation — verified live that the GET now makes zero database change. This stood on its own regardless of the token project and shipped as its own commit (`7a9c964`).

`migration_add_owner_access_token.sql` written (not run at the time): one new column, `owners.access_token text NOT NULL DEFAULT gen_random_uuid()::text UNIQUE` — no separate index needed, since a `UNIQUE` constraint always auto-creates its own backing index, confirmed as documented Postgres behavior rather than assumed.

### Phase 3 — the build

Every dog/owner-scoped route now authorizes on **token OR owner-session cookie**, not token alone — a deliberate design choice, not scope creep: the existing Aug 22 owner-session cookie was built purely additive (it "never gates access to anything"), and hard-requiring a token as the *only* path would have broken every link that cookie's own dog-switcher, "Back to Dashboard," and post-signup redirects already relied on, none of which were ever built to carry a token. `authorizeOwnerScope()` (new shared helper) checks the session first, falls back to a live `access_token` comparison. Making this *hard* on all 12 routes required propagating the resolved token into every internal link/fetch those routes themselves render too — the dashboard's own action links, the medication pending-banner buttons, `add-dog.html`'s own JS — via one shared `withToken()` helper, so a visitor who arrived via a mailed token link (no session cookie) doesn't hit a dead end on their very next click.

- **All 6 (really 8) link-construction sites** now embed the owner's live `access_token`.
- **All 12 access-check routes** — 4 read-only, 8 write-capable, including the explicitly prioritized `POST /api/add-dog` — now reject a missing or wrong token/session with a clean `403` (a friendly HTML page pointing to `/recover.html` for GET routes, a JSON error for API routes), never a raw crash. `POST /api/notes/:dog_id` picked up a second, real fix in the same pass — it previously had no dog-existence check at all.
- **A new public recovery flow** (`Public/recover.html`, `POST /api/recover-account`, `GET /recover`) — phone-only lookup, a short-lived single-use token deliberately separate from `access_token` itself (recovery gets someone back to their existing real link; it doesn't rotate it), sent to both phone and email once a match is found, reusing the exact rate-limiting and anti-enumeration posture already proven by `/api/resend-dashboard-link`.
- **A new "Regenerate My Link" dashboard button** (`POST /api/regenerate-access-token`) — deliberately authorizes on the session cookie *alone*, not the token/session OR-path every other route uses, since allowing a token to authorize its own replacement would let anyone merely holding a leaked link rotate it themselves.

### A real gap found in review, before anything was committed: the session cookie was forgeable

`OWNER_SESSION_COOKIE` was the bare, plaintext `ownerId` — unsigned. Two consequences: regenerating `access_token` had zero effect on any session cookie already sitting in another browser (the exact lost/stolen-device scenario "Regenerate" exists to defend against kept full access after the button was clicked); and since `ownerId` is not itself secret (it's in plain sight in every `/checkins/:owner_id` link ever sent), anyone who had ever seen any link for a given owner could forge a valid session cookie themselves — a real authentication bypass inherited the moment the session cookie became an alternative full-access path. Fixed by HMAC-signing the cookie (`${ownerId}.${signature}`, keyed by both the owner and their live `access_token`, using a new server-only `SESSION_SIGNING_SECRET`), with the verification comparison required to use `crypto.timingSafeEqual` — not `===` — per this project's own prior timing side-channel finding (Aug 29, the `find-my-dashboard` lookup). Verified live: the actual bug proven fixed, not just proven to compile — a real cookie that worked one request earlier correctly returned `403` on the very next request immediately after regenerating; five distinct forged/malformed cookie shapes all correctly rejected with none crashing.

### Full production verification — every check run against the real live server, not localhost

Pushed and deployed via Railway. Confirmed on production, not inferred from the local pass: the app booted clean with `SESSION_SIGNING_SECRET` (added to Railway separately) actually loaded; a real disposable test owner/dog created directly against production; a genuine token-based link worked; a real session cookie was minted and worked; regenerating the token correctly invalidated **both** the old token and the old session cookie in the same real request cycle — the feature's actual core claim, proven in the real environment; the recovery flow sent a real Twilio send (using the reserved test number to avoid a real message to a real number); all test data deleted and confirmed at 0 afterward. All 7 required production checks passed.

### Two items deliberately left open, recorded rather than lost

- **`authorizeOwnerScope`'s plain `===` token comparison** — the same *class* of non-constant-time comparison as the session-signature bug just fixed, deliberately not touched in the same pass. Reasoning: `access_token` is a 36-character random UUID already transmitted in plaintext by design (SMS, email, URLs) — its threat model is "hard to guess," not "resistant to being observed," unlike the session signature, which should never leave the server and where the cookie's own plaintext `ownerId` gave a real non-random anchor to time against. Lower priority, not urgent; revisit if rate-limiting posture on token-accepting routes ever changes materially.
- **`npm audit`: 3 moderate `qs` vulnerabilities**, pulled in transitively through `body-parser` → `express`. `npm audit fix` (run, not forced) made zero changes — traced to the real cause: the patched `qs` version requires `body-parser@2.x`, which requires `express@5.x`, a major framework upgrade outside this build's scope. Two real remediation paths recorded, neither taken: force the full Express 5 upgrade (real regression risk, needs its own pass), or a targeted `overrides` pin on `qs` alone (lower blast radius, still needs its own verification). Moderate severity, not urgent enough to block deploy on its own.

**Supporting services/tools:** `server.js`, `Public/recover.html`, `Public/add-dog.html`, `migrations/migration_add_owner_access_token.sql`, `migrations/migration_add_owner_recovery_tokens.sql`, Railway (new `SESSION_SIGNING_SECRET` env var), Twilio.

---

## September 3, 2026 (continued) — Privacy Policy & Terms of Service Rewrite, Published Live; FAQ Brought Into Alignment

Full investigation findings and every verification pass live in `Legal_Pages_Publish_Build.md` — not duplicated here in full.

### Investigation surfaced a stale premise before any content changed

The reviewed rewrite drafts in `docs/LEGAL-DRAFTS/` (Privacy Policy and Terms of Service, both marked "DRAFT — requires attorney review") described persisting a signup consent record as future work still needed. **It wasn't** — `consent_given_at` and `consent_policy_version` were already live and correctly wired on both real signup paths, shipped the day before (see the policy-version entry above). This changed the actual scope of the work from "content plus backend" to "a content and template replacement on top of an already-sound system" — flagged clearly before any page was touched, since building the described-as-missing mechanism a second time would have been real wasted work chasing a stale doc.

### Both pages rewritten and published

`privacy.html` was rebuilt onto `terms.html`'s existing dense numbered-section template (shared `.legal-content`/`.highlight` styling) rather than the reverse, per direction — `terms.html`'s own structure was left untouched. Full 14-section body of each replaced verbatim with the reviewed draft content (DRAFT banner and internal "Open Placeholders" review notes stripped from both). Governing law kept as drafted — State of Texas, confirmed factually correct before shipping. Dates updated to September 2, 2026; `privacy.html`'s contact email converted to a real `mailto:` link, matching `terms.html`'s existing pattern. Every internal same-page section cross-reference in the drafts (six of them) was checked against the actual numbering shipped — all correct, since numbering wasn't reordered.

### The signup consent checkbox strengthened, and a real bug found via a real click, not a diff review

The consent checkbox on both `baseline-health-journey.html` and `add-dog.html` now names and links the real Privacy Policy and Terms of Service directly (`target="_blank" rel="noopener"`, so checking a policy mid-signup doesn't discard the in-progress form). **A real bug then surfaced from an actual manual click, not a DOM-attribute check** — clicking "Privacy Policy" discarded the form and landed on `trust-and-data.html` instead of `privacy.html`. Investigated rather than guessed at: confirmed via `curl` against the real running server that the served HTML matched the repo exactly, confirmed no click handler anywhere touches this link, then directly reproduced the exact symptom by clicking a *different*, pre-existing link 176px below the checkbox — a "Learn more." caption under the submit button, and a second, similar "See our Terms & Privacy" link near the SMS field — both pointed at `trust-and-data.html` with no `target` attribute at all. The actual link built for this task was correct throughout. Both pre-existing links fixed in a follow-up pass: "Terms & Privacy" split into two separately-linked words, "Learn more." repointed at `privacy.html`, both now carrying the same `target="_blank" rel="noopener"` treatment.

### FAQ verbiage pass — six real discrepancies found and fixed across 8 answer blocks

With the real legal pages now live and specific, a systematic cross-check of every `faqs.html` entry touching data sharing, consent, retention, account access, opt-out, medical disclaimers, and jurisdiction against the actual live policy text surfaced six real gaps — none outright false, all materially vaguer or softer than what the site now explicitly commits to in writing (the same class of issue as the stale consent-record premise above): `#f6` claimed notes weren't used for intelligence "at this time" where the real policy makes this a *permanent*, architecturally-enforced exclusion, and listed a far narrower collected-field set than actually disclosed; the `#f5`/`#f7`/`#f18` cluster never named which categories of company (pharma, research, supplement, food, insurance, product manufacturer, retail) actually license the de-identified data, despite the real policy stating this plainly as the core business model; `#f21` implied participation was separable from data licensing when the real policy bundles them; `#f8` never addressed that de-identified data is retained indefinitely even after identifying info is deleted; `#f11` didn't mention the passwordless, link-as-password account model; `#f15` referenced "our Privacy Policy" as plain text instead of a real link. All six rewritten to match the live policy's specificity, live-verified through the page's real accordion toggle (open and close, all 8 answers, zero console errors) and a real click test on the new link.

Both efforts — the legal-page rewrite and the FAQ alignment — were pushed and confirmed live on production via direct `curl` checks (content markers, section counts, link destinations) rather than assumed from a successful `git push`.

**Supporting services/tools:** `Public/privacy.html`, `Public/terms.html`, `Public/faqs.html`, `Public/baseline-health-journey.html`, `Public/add-dog.html`, Railway.

---

## September 4, 2026 — Signup Survey Fixes: Age Under 1 Year, Notes Disclosure, Treatment/Medication Merge

Three related fixes to both real signup surfaces (`baseline-health-journey.html`, `add-dog.html`), shipped as three separate, individually reviewable commits after an investigation-only pass confirmed exactly what was missing on each form before any code changed.

### Age field: no way to register a dog under 1 year old, now fixed — plus a second legacy CHECK constraint found the same way STEP P10's `mobility_check` was

The signup age field forced a whole-number "age (in years)" input with a hard client- and server-side minimum of 1 — there was no way, through either signup form, to register a genuine puppy under 12 months old. This directly contradicted the Aug 20 decision to keep data collection open to all ages (senior stays the marketing/pitch wedge; the data model itself was never meant to gate on age).

Replaced the single age input with two fields — Years, and a Months (0-11) dropdown — on both forms. Server-side, `/api/send-magic-link` and `/api/add-dog` now validate years and months separately, with a floor requiring at least one to be nonzero, replacing the old `!age`/`cleanAge < 1` truthy checks that would have wrongly rejected a real, valid `age_years: 0` submission the same way the old single-field design already did. New `senior_dogs.age_months`/`magic_link_tokens.age_months` columns (`migration_add_dog_age_months.sql`, run). New shared `getDogAgeInYears(dog)` (returns a fractional-year value, e.g. `0.67` for 8 months) and `formatDogAge(years, months)` (renders "8 months old," not "0 years old") helpers — all 3 existing `isSeniorForBreed()` call sites (breed guide route, dashboard route, `buildPetProofingHtml`) and all 3 age-display sites (dashboard header, dashboard Health Journey section, Journey Summary modal) updated to use them instead of the old raw `dog.age` value.

**A real bug caught in review before anything shipped**: the first pass of `/api/add-dog`'s rewrite left its Google Sheets export line still referencing the now-renamed `cleanAge` variable — would have thrown a `ReferenceError` on every real add-dog submission. Caught via a deliberate grep sweep after `node --check` (which only validates syntax, not undefined-variable references) passed clean — the same class of gap this project's own `Health_Instrument_Redesign_Build.md` had already flagged as worth checking for explicitly. Fixed before the diff was ever shown for review.

**Two previously-unknown legacy CHECK constraints found empirically, the same class of surprise as STEP P10's `mobility_check` incident (Aug 23)** — neither documented anywhere, neither touched by the migration that added `age_months`, both found only through real testing with a genuine edge-case value rather than a safely-mid-range one:

1. A first migration attempt to allow `age: 0` on `senior_dogs` surfaced a pre-existing range constraint on that column, worked around by the two-field (years/months) design itself rather than requiring its own removal.
2. After that migration ran, a real submission for a genuine 0-years/8-months puppy through `/api/send-magic-link` still failed — `new row for relation "magic_link_tokens" violates check constraint "age_check"`. Investigated empirically before writing any fix, not assumed: disposable probe inserts confirmed `age: 0` and `age: -1` both rejected, `age: 1` accepted — the original, pre-this-change "age must be at least 1" rule, still live on `magic_link_tokens` specifically. Confirmed via a separate probe that `senior_dogs.age` carries no such constraint — isolated to the staging table. Fix written as `migration_drop_legacy_age_check.sql`, not run at the time per standing project rule (John runs migrations manually) — confirmed run by John in a later message, then independently re-verified live via a fresh probe insert before any real signup was attempted against it.

### Notes-field disclosure

A short in-context disclosure — "This isn't reviewed or filtered — avoid including anything you wouldn't want stored as plain text." — added directly under the observations textarea on both signup forms, exactly where the Aug 20 strategic-decision session (see `CompanionCommons_Strategy_and_Legal_Aug20.md` Section 8) had called for it. Confirmed missing during investigation before writing anything — the decision had been made and partially hardened (the equivalent disclosure already existed on `privacy.html` itself) but this specific in-context copy had never actually been built on either form. The matching disclosure for the medication "other" free-text field shipped in the next commit instead of this one, since that field's markup was being rewritten there anyway.

### Treatment checkboxes and medication entries merged — the same fact was being stored twice with no reconciliation

The 7 treatment-type checkboxes (Joint supplement, NSAID, Steroid, Pain medication, Other prescription medication, Other supplement, None) and a separate, always-visible Medications & Supplements section both existed on both forms, independently — checking "NSAID" up top and adding a full NSAID entry in the section below both wrote to `treatment_category`, with nothing tying the two together and no reconciliation if they disagreed.

Redesigned so checking a non-"none" checkbox reveals an inline entry section locked to that category — a hidden `.med-category` input rather than the old visible category dropdown, since the category is now implied entirely by which checkbox triggered the section. Supports multiple entries per category via a "+ Add another" button. Unchecking a box that already has a filled-in entry now calls `confirm()` before discarding the data, matching this form's existing "fail loudly, don't silently drop" instinct rather than quietly losing what was typed. The old, separate, always-visible Medications & Supplements section is gone entirely. `treatment_category` is now derived client-side at submit time from which categories actually have a filled entry, not from the checkboxes' own state directly — confirmed via reading every real consumer of `treatment_category` (dashboard display, Google Sheets export, server-side validation) before shipping this that the array's shape and valid values were completely unchanged, meaning this required zero server-side changes.

**Live-verified against the real running server**, both branches of the confirm dialog independently checked (native `confirm()` dialogs are suppressed in this session's browser sandbox, confirmed via a console warning rather than assumed — the Cancel path was verified against that real suppressed-`false` behavior, and the OK path verified with a temporary `window.confirm` stub): checking a box reveals exactly one pre-categorized entry with no visible category field; "+ Add another" adds a second entry in the same category; the real submit request's payload was intercepted via `fetch` interception and confirmed `treatment_category` derives correctly from the filled entries on both signup surfaces.

### Commit surgery: one combined working-tree diff split into 3 reviewable commits, verified byte-identical

All three pieces above were built and reviewed together as one large diff (per instruction, to show the full change before committing anything), then split into three separate, cleanly-scoped commits — `d392ea1` (age fix + both migrations), `42854fd` (notes disclosure), `7abe58b` (treatment/medication merge) — via the same revert-then-reapply commit-surgery pattern already established in `Link_Revocation_Build.md`: a pre-surgery ground-truth diff and full-file snapshots were captured first, each commit's own region reverted and reapplied in isolation with intermediate-state diffs checked against the snapshots at every step, and the final combined diff across all three commits was compared byte-for-byte against the original pre-surgery ground truth — confirmed identical before anything was pushed. A self-introduced stray `</fieldset>` tag (from an Edit tool call whose `old_string` hadn't included a trailing closing tag belonging to the old Medications & Supplements fieldset) was caught via a `<fieldset>`/`</fieldset>` count mismatch and fixed before the diff was ever shown for review, not after.

### Live re-verification once the second migration ran, and a separate production render/structure check

Once John confirmed `migration_drop_legacy_age_check.sql` had been run, the exact 0-years/8-months puppy test was re-run for real through `baseline-health-journey.html` specifically — the main new-owner signup path, not the `/api/add-dog` bypass used during development to make progress around the still-pending constraint. A real form fill (via the actual page's own DOM, dispatching real `input`/`change` events, catching one real test-setup mistake along the way — the diet-type `<select>`'s real option value is `"dry"`, not `"dry_kibble"`, caught via a form-validation highlight rather than assumed correct) and a real click on the real submit button produced a genuine `POST /api/send-magic-link` success, a real `/verify` redirect, and a real `senior_dogs` row confirmed at `age: 0, age_months: 8`. `formatDogAge()` confirmed rendering "8 months old" (not "0 years old") on the dashboard header, dashboard Health Journey section, and Journey Summary modal; `isSeniorForBreed()` confirmed correctly evaluating false (no senior badge anywhere); the breed guide page loaded cleanly with no crash, correctly showing its locked-until-week-2 state for a fresh signup. All test data deleted and confirmed at 0 across every table touched.

Separately, after both commits were pushed and Railway auto-deployed, a **structure-only check against production** (no real signup submitted, per explicit instruction) confirmed both forms render correctly on the live site: the age Years/Months fields, the notes disclosure (present twice per form — main observations field and medication "other" field), and all 6 treatment-category entry blocks (`joint_supplement`, `nsaid`, `steroid`, `pain_medication`, `other_prescription`, `other_supplement`), each starting `hidden`, with the `rowHasAnyData`/`addMedicationRow` JS functions and the confirm-before-discard guard all present. The old single age field and the old always-visible medication section were confirmed gone. Since production is behind `SITE_PASSWORD`, this check required unlocking the live gate — done via a one-way SHA-256 hash of the real password (John ran a local `curl` command and shared back only the resulting `cc_site_access` cookie value, never the raw password itself), with the cookie's name and 64-char hex shape validated against `server.js`'s own `SITE_UNLOCK_COOKIE`/`siteUnlockHash()` implementation before it was ever used.

**Supporting services/tools:** `server.js`, `Public/baseline-health-journey.html`, `Public/add-dog.html`, `migrations/migration_add_dog_age_months.sql`, `migrations/migration_drop_legacy_age_check.sql`, Railway.

---

## September 4, 2026 (continued) — authorizeOwnerScope Timing Side-Channel Closed

Closes the `authorizeOwnerScope` backlog item flagged (and deliberately deprioritized) during the Sep 3 link-revocation build's own security sweep — see `Link_Revocation_Build.md`'s Backlog section for the original reasoning on why this was left for later rather than fixed alongside the session-cookie signature bug at the time.

**The fix**, matching the exact pattern already used for the session-cookie signature check earlier in that same build: the plain `realToken === providedToken` comparison replaced with `crypto.timingSafeEqual`, guarded by a length check first (`timingSafeEqual` throws on a length mismatch rather than returning `false`, so unequal-length buffers are rejected before ever reaching it — a stray non-constant-time length comparison is an acceptable leak, since it only reveals "was the length right," never anything about the token's actual content).

**Live-verified against the real running server**, one disposable owner/dog created with a real `access_token`: the correct token authorized successfully (`200`); a wrong token of the identical length was correctly rejected (`403`); wrong-length tokens, both shorter and longer than the real token, were correctly rejected (`403`), confirmed via the server's own error logs that this was the length guard catching it, not `timingSafeEqual` throwing into an unhandled crash; a missing/empty token was correctly rejected (`403`). A file-wide grep confirmed the function's rewritten body contains zero remaining `===` comparisons of token content, and that no other place in `server.js` ever compared `access_token`/`realToken`/`providedToken` with `===` — this was the only such site in the whole file. Test owner/dog deleted, confirmed at 0.

**Supporting services/tools:** `server.js`.

---

## September 4, 2026 (continued) — qs/body-parser Moderate Vulnerability Fixed via a Targeted Overrides Pin

Closes the second Link_Revocation_Build.md backlog item — 3 moderate `npm audit` findings in `qs` (an array-limit bypass, a DoS via attacker-controlled `isBuffer`), pulled in transitively through `body-parser` → `express`. `npm audit fix` had already been confirmed (during the original link-revocation sweep) to make zero changes, since the patched `qs` version requires `body-parser@2.x`, which requires `express@5.x` — a major framework upgrade judged out of scope for a moderate-severity, pre-beta fix at the time.

**Resolved instead with a targeted `package.json` `overrides` entry** — `"overrides": { "qs": "6.16.0" }` — pinning the patched `qs` version directly while leaving Express 4 and `body-parser` 1.x completely untouched. Confirmed the override actually took, not just that `npm install` completed without error: `node_modules/qs/package.json` reports `"version": "6.16.0"`, and `npm audit` now reports 0 vulnerabilities (down from 3).

**Real verification, since this overrides `body-parser`'s own declared compatibility range rather than a change either package made itself**: started the real dev server and exercised both real code paths in this app that route through `qs` — a `GET` request with bracket/array-notation query params (`?a[b]=1&arr[]=x&arr[]=y`, the same shape the array-limit-bypass advisory concerns; a first attempt hit a curl URL-globbing artifact from the unescaped `[`/`]` characters, resolved with `--globoff`, not a real server issue) returned a correct `200`; a `POST` with a JSON body and a separate `POST` with an `application/x-www-form-urlencoded` body (`bodyParser.urlencoded({ extended: true })` is specifically what pulls `qs` into the urlencoded-parsing path) both returned correct `200` responses with the submitted field correctly extracted into `req.body`. Zero errors in the server logs across all three. Went one step further than "didn't crash": called the pinned `qs` module directly (`qs.parse('a[b]=1&a[c]=2&arr[]=x&arr[]=y')`) and confirmed it produces the exact expected nested-object/array shape (`{a:{b:'1',c:'2'}, arr:['x','y']}`), not merely a non-error result. `node --check server.js` passed and a clean local boot was confirmed before considering this closed.

**Explicitly not done, and not implied by this fix**: the Express 5 / `body-parser` 2 upgrade remains a separate, deliberately deferred future decision — this closes only the specific `qs` advisory.

**Supporting services/tools:** `package.json`, `package-lock.json`.

---

## September 4, 2026 (continued) — Legal-Rewrite Process Notes Archived

`docs/LEGAL-DRAFTS/Privacy_Terms_Rewrite_Outline (1).md` — the working outline behind the Sep 3 Privacy Policy/Terms of Service rewrite, flagged Sep 3 as a real, undecided repo-hygiene loose end (its own "Summary of Real Open Items" list had already gone stale in at least one place, since the rewrite it was written to drive had shipped and gone live) — renamed to `docs/LEGAL-DRAFTS/Legal_Pages_Rewrite_Process_Notes.md`, with a short archival note prepended above the original title: every 🔴-flagged open item in the outline was resolved in the final shipped drafts (both live in production as of Sep 3), and the file is kept for reference on how those drafts were built, not as a source of current open questions.

Since the entire `docs/LEGAL-DRAFTS/` directory had never been committed to begin with, this landed in git as a plain file addition rather than a detected rename — flagged directly before committing, since there's no prior git history for a rename to actually preserve here, only the practical rename-and-annotate outcome itself.

**Supporting services/tools:** `docs/LEGAL-DRAFTS/Legal_Pages_Rewrite_Process_Notes.md`.
