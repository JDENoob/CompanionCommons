...
**Second confirmed instance, Batch 3 test cleanup (Sep 4): `health_alerts` has the same RESTRICT (not CASCADE) shape.** A real health alert fired during Batch 3's reminder-logic test (a dog's first real check-in moved its mobility composite by exactly the 1.0 threshold from baseline), and the resulting `health_alerts` row blocked the same `DELETE FROM senior_dogs` the exact same way `dog_notes` did in Batch 1 — same `23503` error, same fix (delete `health_alerts` for the dog first). **Updated guidance for Phase 5 and any future cleanup logic:** at minimum `dog_notes` and `health_alerts` are RESTRICT-linked to `senior_dogs` and must be deleted first; do not assume any other table is CASCADE-linked either without checking — confirm each one empirically (as both of these were) rather than assuming the medication-family tables' CASCADE behavior is the norm across the whole schema.

---

## Project status: ✅ Complete — all 5 phases built, verified, and shipped

**Phase 3** created `owner_pet_links` (schema + the 4 named-function convention) and wired it into a real dual-write at both dog-creation sites (`/verify`, `/api/add-dog`), alongside `senior_dogs` — verified live via a real FK-violation rollback test and a real happy-path signup, with zero behavior change to anything reading `senior_dogs`.

**Phase 4** cut every real reader over to the new table across three batches: Batch 1 (6 simplest access-control checkpoints), Batch 2 (the 4 `select('*')` display routes' primary auth checks, plus finding and fixing a real, unrelated Sep 3 session-signing regression in the dashboard's dog-switcher along the way), and Batch 3 (the dashboard sibling-dog query, `/api/checkin-senior`'s reminder-queueing logic, and the ops/governance SMS opt-in stat via two new aggregate/bulk helpers). Every batch was verified with real disposable data and byte-for-byte before/after parity via `git stash`.

**Phase 5** finished the separation in three steps: Step 1 narrowed the 4 `select('*')` routes to explicit column lists (catching one real correction the original Phase 2 sketch missed — `/api/checkin-senior` needed the mobility item fields, not just the composite). A pre-flight investigation for Step 2 then surfaced **two real, previously-missed consumers** still reading `senior_dogs`' identity columns directly — the live hourly churn-detection cron and `GET /checkins/:owner_id`'s sibling-dog query — cut over in Step 2b (the churn cron via a new 3-query bulk-resolution helper, `bulkResolveOwnerContactForDogs`, designed and reviewed before implementation given it touches live SMS/email delivery). Step 2 (final) then stopped writing all 5 identity columns to `senior_dogs` at both creation sites — confirmed via a fresh full-codebase grep immediately beforehand that no other reader existed. Step 3 dropped the 5 columns for real in production (`migration_drop_senior_dogs_identity_columns.sql`).

**Final end-to-end verification, against the real post-drop production schema, all passing:** a real signup through `/verify` and a real second dog through `/api/add-dog`, both against the now-columnless `senior_dogs`; `/checkins/:owner_id` and the dashboard switcher correctly listing both dogs; the real churn-detection cron (backdated test dogs, the actual extracted evaluation function) correctly resolving owner/phone/email through `owner_pet_links` and correctly grouping sibling dogs; and both `/api/governance/stats` and the ops dashboard's headline metrics reporting a correct, hand-verified SMS opt-in rate. `senior_dogs` now carries zero owner-identifying data for any dog created from this point forward — `owner_pet_links` (dog-to-owner, per-dog `sms_consent`) and `owners` (contact info) are the sole sources of identity, exactly as scoped at Phase 1.
