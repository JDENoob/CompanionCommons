...
**Real bug found and fixed on review, not part of the original design brief:** `buildDogSwitcherHtml` was interpolating `d.dog_name` into each tab's HTML unescaped — a stored-XSS gap, same class already known and still open in the Journey Summary's note rendering. Confirmed `escapeHtml()` was already defined in `server.js` (used elsewhere for notes and chart labels) and applied it here. While fixing this, checked whether the existing, unrelated `<h1>${dog.dog_name}'s Mobility Dashboard</h1>` line in the same route has the identical gap — it does, confirmed, but left unfixed as genuinely out of scope for this change. This makes three independently-found instances of the same unescaped-interpolation pattern (Journey Summary notes, this switcher tab, this heading), each caught by accident rather than a deliberate sweep — logged as its own "Still Open" item in `CompanionCommons_Build_Log.md` calling for a real audit rather than more one-off fixes.
